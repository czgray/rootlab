...
def extract_sample_id(name: str) -> str:
    # Match canonical IDs followed by a dash (with or without spaces)
    m = re.match(r"^([A-Za-z]+\d+[A-Za-z]+\d?)\s*-\s*", name)
    if m:
        return m.group(1)
    # fallback: return unchanged if not matching canonical pattern
    return name
...
def summarize_branches(
    ...
    # Two ways of creating volume: by segment then aggregated, or by branch
    df['volume_cm3_by_branch'] = np.pi * (df['width_cm'] / 2.0) ** 2 * df['length_cm']
    df["volume_cm3"] = df["branch_volume_px3"] / (px_per_cm ** 3)
    ...
    total_vol_cm3_seg = g["volume_cm3"].sum().rename('total_vol_cm3_seg')
    total_vol_cm3_branch = g['volume_cm3_by_branch'].sum().rename('total_vol_cm3_branch')
    ...
    if by_order:
        ...
        # --- volume proportions (by segment- and branch-based volume) ---
        vol_by_order_seg = g["volume_cm3"].sum()
        total_vol_seg_by_group = vol_by_order_seg.groupby(level=base_cols).transform('sum')
        vol_prop_seg = (vol_by_order_seg / total_vol_seg_by_group).rename('vol_proportion_seg')
    
        vol_by_order_branch = g["volume_cm3_by_branch"].sum()
        total_vol_branch_by_group = vol_by_order_branch.groupby(level=base_cols).transform('sum')
        vol_prop_branch = (vol_by_order_branch / total_vol_branch_by_group).rename('vol_proportion_branch')

        
        summary_df = pd.concat(
            [total_length, avg_width, branching_intensity, length_prop, vol_prop_seg,vol_prop_branch,
             total_vol_cm3_seg, total_vol_cm3_branch],axis=1).reset_index()

    else:
        summary_df = pd.concat(
            [total_length, avg_width, branching_intensity, total_vol_cm3_seg, total_vol_cm3_branch],axis=1).reset_index()
