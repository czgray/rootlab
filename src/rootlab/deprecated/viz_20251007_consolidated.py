...
class _VizConfig:
    ...
    figure_dpi: int = 110   # used when showing
    save_dpi: int = 450     # used when saving
...
def _apply_fig_defaults(figsize=None, dpi=None):
    # dpi here = figure (show) dpi; save uses _VIZ.save_dpi later
    return {"figsize": figsize or _VIZ.figsize, "dpi": dpi or _VIZ.figure_dpi}
...
def plot_sample_by_order(
    ...
    *,
    ...
    """Plot sample(s) colored by order (biological or DFS). Uses batched drawing and _render_core."""
    ...
    for sid in _iter_samples(sample_id, segment_maps):
        ...
        # Resolve output path (directory-aware) and render
        out_path = save_path
...
def plot_sample_by_system(
    ...
    *,
    ...
    """Plot sample(s) colored by system (tab10). Uses batched drawing and _render_core."""
    required = set(_REQUIRED_BASE)
    _validate_required_columns(branch_table_df, required)
    ...
    *,
    ...
    Selection semantics:
      - sample_id can be:
          ...
          * Iterable[str]: render each sample in the iterable (one figure per sample)
    ...
    Candidate selection is keyed by (sample_id, system_index) in `taproot_results`.
    Background systems are stroked in gray using true per-segment widths.
    Candidate masks use true per-segment widths; the candidate polyline uses a fixed stroke.
    """
    import json
    ...
    # Decide which sample ids to render
    if sample_id is None:
    ...
    # Helper to resolve output path per-sample
    def _resolve_out_path(base_path: Optional[str], sid: str, sys_tag: str) -> Optional[str]:
        ...
        # If a single file path was provided but multiple samples are rendered,
        # append the sample/system tag before the extension to avoid collisions.
    ...
    for sid in sample_ids:
        ...
        if system_index is None:
            ...
            # If the provided system_index is out of range for this sample, warn and skip
        ...
        for i, cand in enumerate(candidates):
            ...
            # Legend entry: only candidate number
