...
# -----------------------------------------------------------------------------
# New Helpers: Integrate into the rest!!
# -----------------------------------------------------------------------------
def _coerce_segmap(obj):
    # canonical dict
    if isinstance(obj, dict):
        return obj
    # common packaging: (segment_map_dict, links_or_meta)
    if isinstance(obj, tuple) and obj and isinstance(obj[0], dict):
        return obj[0]
    # sometimes wrapped: {"segment_map": {...}}
    if isinstance(obj, dict) and "segment_map" in obj and isinstance(obj["segment_map"], dict):
        return obj["segment_map"]
    return obj
