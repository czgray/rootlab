...
# add near the top of viz.py with other imports
...
class _VizConfig:
    ...
    figure_dpi: int = 120     # DPI for interactive figures (plt.show)
    save_dpi: int = 450       # DPI for saved outputs (savefig)

    # Stroke/width policy
    min_linewidth: float = 0.5
    width_scale: float = 0.5  # global multiplier applied to (width_right - width_left)
    width_max: float = 8.0    # clamp ceiling after scaling
    ...
    # --- Pixel-accurate rendering flag ---
    pixel_accurate: bool = False # if True, use data_width_to_points for linewidths
...
def data_width_to_points(ax, data_width: float) -> float:
    ...
    lw = data_width * 72.0 / ax.figure.dpi
    # print(f"→ {data_width:.2f}px → {lw:.2f}pt at dpi={ax.figure.dpi}")
    
    # return display_dist_px * 72.0 / fig.dpi
    return max(_VIZ.min_linewidth, min(lw, _VIZ.width_max)) # Cap minimum and maximum
...
def _collect_segment_drawables(segmap, seg_ids, invert_y):
    """Prepare segment points and raw pixel widths (not converted to points)."""
    seg_arrays = []
    widths_px = []
    for sid in seg_ids:
...
def _render_core(
    *,
    ...
    # Adding color legend to the side
    cbar_smap: Optional[ScalarMappable] = None,
    ...
    # --- Pixel-accurate width conversion ---
    if _VIZ.pixel_accurate:
        # Convert each raw pixel width to display points using the current Axes transform
        widths_pts = [data_width_to_points(ax, w_px) for w_px in widths]
    else:
        widths_pts = [
            max(_VIZ.min_linewidth, min(w_px * _VIZ.width_scale, _VIZ.width_max))
            for w_px in widths
        ]
    ...
    if base_segments:
        base_widths_pts = (
            [data_width_to_points(ax, w_px) for w_px in base_widths]
            if (_VIZ.pixel_accurate and base_widths)
            else [
                max(_VIZ.min_linewidth, min(w_px * _VIZ.width_scale, _VIZ.width_max))
                for w_px in (base_widths or [])
            ]
        )
        lc_bg = LineCollection(
            base_segments,
            colors=[_VIZ.base_gray] * len(base_segments),
            linewidths=base_widths_pts,
    ...
    # Foreground layer
    lc = LineCollection(segments, colors=colors, linewidths=widths_pts, zorder=2)
    ...
    # Add color bar on the righthand side
    if cbar_smap is not None:
        divider = make_axes_locatable(ax)
        cax = divider.append_axes("right", size="3%", pad=0.25)
        cb = plt.colorbar(cbar_smap, cax=cax, ticks=cbar_ticks)
        if cbar_label:
...
def plot_sample_by_order(
    ...
    *,
    ...
    for sid in _iter_samples(sample_id, segment_maps):
        ...
        # Build colorbar scalar mappable (continuous mapping)
        arr = np.asarray(order_vals, dtype=float)
        ...
        cmap = _get_cmap(_VIZ.cmap_order)
        norm = Normalize(vmin=vmin, vmax=vmax)
        smap = ScalarMappable(norm=norm, cmap=cmap)
        
        # Choose ticks: integers if all values are (close to) ints and range is small
        ticks = None
        if finite.size and np.allclose(finite, np.round(finite)) and (vmax - vmin) <= 12:
            ticks = list(map(int, sorted(set(int(x) for x in finite))))
        
        cbar_label = "DFS Order" if mode.lower() == "dfs" else ("Biological Order (flipped)" if use_flipped_order else "Biological Order")
