    ...
    try:
        ...
        for sid, seg in segmap.items():
            ...
            if cov >= overlap_threshold or min_dist <= (halfw + 0.5 + 0.5 * seg_width):
                # Angle filter (exclude lateral branches)
                ang = compute_angle(seg["points"], AL_dense)
                if ang is None or abs(ang) <= 45:  # 45° cutoff for lateral branches
                    kept_ids.append(sid)
