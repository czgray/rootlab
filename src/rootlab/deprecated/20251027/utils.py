...
def summarize_branches(
    ...
    if by_order:
        ...
        summary_df = pd.concat([total_length, avg_width, branching_intensity, length_prop], axis=1).reset_index()
    else:
        summary_df = pd.concat([total_length, avg_width, branching_intensity], axis=1).reset_index()
...
def load_taproot_annotations(path: str | Path) -> dict[str, dict]:
...
def merge_taproot_annotations(existing_path, new_results):
    """
