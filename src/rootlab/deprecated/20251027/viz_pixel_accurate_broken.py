...
class _VizConfig:
    ...
    # --- NEW: pixel-accurate rendering controls ---
    pixel_accurate: bool = True
    pixel_mode: str = "screen"    # "screen" | "physical" | "custom"
    pixel_scale: float = 1.0      # used only if pixel_mode == "custom"
    image_dpi: int = 600          # used only if pixel_mode == "physical"
...
def data_width_to_points(ax, data_width_px: float) -> float:
    """
    Convert an image width in *pixels* to a Matplotlib linewidth in *points*.

    Modes:
      - Screen 1:1 (default): 1 image px → 1 display px
      - Physical: 1 image px → 1/image_dpi inches (paper-accurate)
    """
    w = float(data_width_px)

    # Choose one policy. Default to screen-accurate 1:1.
    if _VIZ.pixel_mode == "screen":        # 1 image px == 1 display px
        return w * (72.0 / ax.figure.dpi)
    elif _VIZ.pixel_mode == "physical":    # 1 image px == 1/image_dpi inches
        return w * (72.0 / _VIZ.image_dpi)
    else:                                   # custom scale: k display px per image px
        k = float(_VIZ.pixel_scale)         # e.g., k=0.5 shrinks thickness by half
        return w * k * (72.0 / ax.figure.dpi)
...
# ---- Width computation helper
def _width_from_meta(meta, ax=None) -> float:
    wl = float(meta.get("width_left", 0) or 0)
    wr = float(meta.get("width_right", 0) or 0)
    w_px = max(wr - wl, 0.0)

    if _VIZ.pixel_accurate and ax is not None:
        lw = data_width_to_points(ax, w_px)
    else:
        lw = w_px * _VIZ.width_scale
...
# ---- Segment collection with optional Axes context

def _collect_segment_drawables(
    segmap: Mapping[str, Any],
    seg_ids: Iterable[str],
    invert_y: bool,
    ax=None
) -> tuple[list[np.ndarray], list[float]]:
    """Prepare (points, linewidths) tuples for drawing segments."""
    ...
    for sid in seg_ids:
        ...
        widths.append(_width_from_meta(seg.get("meta", {}), ax=ax))
...
def _render_core(
    *,
    ...
    Automatically uses pixel-accurate widths if _VIZ.pixel_accurate=True.
    """
    # Create figure with appropriate DPI
    fig, ax = plt.subplots(figsize=figsize or _VIZ.figsize,
                           dpi=dpi or _VIZ.figure_dpi)
    # The Axes (ax) object defines how data units map to display units.
    # It's passed downstream for pixel-accurate width computation.
    ...
    # --- Pixel-accurate adjustment (optional future extension) ---
    # For now, 1 image pixel == 1 screen pixel. Later, we can apply DPI ratio:
    # scale_factor = (_VIZ.figure_dpi / _VIZ.image_dpi)
...
def plot_sample_by_order(
    ...
    *,
    ...
    for sid in _iter_samples(sample_id, segment_maps):
        ...
        # Create figure + axis here, so we can compute pixel-accurate widths
        fig, ax = plt.subplots(figsize=figsize or _VIZ.figsize, dpi=dpi or _VIZ.figure_dpi)
        
        segments, widths = _collect_segment_drawables(segmap, all_seg_ids, invert_y=inv, ax=ax)
        ...
        _render_core(
            ...
            widths=widths,
            colors=colors,
            title=title or f"{sid} (system {system_id})",
            save_path=save_path,
            figsize=figsize,
            dpi=dpi,
        )
...
def plot_sample_by_system(
    ...
    *,
    ...
    for sid in _iter_samples(sample_id, segment_maps):
        ...
        segments, widths = _collect_segment_drawables(segmap, seg_ids, invert_y=inv, ax=ax)
        
        _render_core(
            segments=segments,
            widths=widths,
            colors=colors,
            title=title or f"{sid} (system {sys_id})",
            save_path=save_path,
            figsize=figsize,
            dpi=dpi,
        )
...
def plot_highlight_segments(
    ...
    *,
    ...
    for sid in _iter_samples(sample_id, segment_maps):
        ...
        # --- Create figure and Axes early so we can compute pixel-accurate widths ---
        fig, ax = plt.subplots(figsize=figsize or _VIZ.figsize, dpi=dpi or _VIZ.figure_dpi)
        
        # Collect drawables using pixel-accurate width conversion
        bg_segments, bg_widths = _collect_segment_drawables(segmap, background_seg_ids, invert_y=inv, ax=ax)
        hi_segments, hi_widths = _collect_segment_drawables(segmap, present_ids, invert_y=inv, ax=ax)
