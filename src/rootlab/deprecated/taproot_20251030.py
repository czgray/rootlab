...
# ---- Core: build taproot guided by clicks ----------------------------------

def build_taproot_from_clicks(
    json_file: str,
    sample_id: str,
    clicks_polyline: List[List[float]],
    params: Optional[TaprootParams] = None,
    eps_px: float = 10.0,
    seg_hit_frac: float = 0.6,
    step_px: float = 20.0,
    ...
    bridge_penalty: float = 25.0,
) -> Dict:
    """
    Map a user-drawn polyline (hand clicks) to the closest coherent path of
    existing segments, allowing short bridges, and return a candidate compatible
    with your current taproot schema.

    Returns a dict with keys:
      - polyline: merged polyline following the chosen segments (list of [x,y])
      - mask_seg_ids: ordered list of segment IDs on the path
      - observed_length_px / fabricated_length_px
      - pct_fabricated
      - depth_along_axis_px
      - score_parts: dict of partial scores (lower is better)
    """
    if params is None:
        params = TaprootParams()

    # 1) Load data & segment map
    img, segment_map = _load_sample(json_file, sample_id)

    # Build adjacency from links present on each root object in the map
    # segment_map is expected as: {seg_id: {"points": [[x,y],...], "meta": {...}, "links": [neighbor_ids]}}
    # If links are not in each segment entry, they should be retrievable from the same JSON via _load_sample.

    # 2) Corridor selection: segments near the clicked polyline
    candidates: List[str] = segments_near_polyline(
        segment_map, clicks_polyline, eps_px=eps_px, seg_hit_frac=seg_hit_frac
    )
    candidate_set = set(candidates)

    # 3) Build subgraph with optional short bridges
    G = nx.Graph()

    def neighbors(seg_id: str) -> List[str]:
        return segment_map.get(seg_id, {}).get("links", [])
    ...
    # Add edges between candidate neighbors
    for sid in candidates:
        for nb in neighbors(sid):
            if nb in candidate_set:
                G.add_edge(sid, nb, bridge=False)

    # Allow short bridges via one intermediate segment (max_bridge_hops=1)
    if max_bridge_hops >= 1:
        for sid in candidates:
            for nb in neighbors(sid):
                if nb not in candidate_set:
                    # one-hop bridge to any candidate next to nb
                    for nb2 in neighbors(nb):
                        if nb2 in candidate_set and nb2 != sid:
                            # create a virtual edge with a bridge flag
                            G.add_edge(sid, nb2, bridge=True, via=nb)

    # If graph is empty, bail out gracefully
    if G.number_of_nodes() == 0:
        return {
            "polyline": clicks_polyline,
            "mask_seg_ids": [],
            "observed_length_px": 0.0,
            "fabricated_length_px": 0.0,
            "pct_fabricated": 0.0,
            "depth_along_axis_px": _polyline_length(clicks_polyline),
            "score_parts": {"note": "no segments found near clicks"},
        }

    # 4) Compute axis and per-node attributes
    origin, axis = _axis_from_polyline(clicks_polyline)
    clicks_samples = _sample_polyline(clicks_polyline, step_px=step_px)

    node_attr = {}
    for sid in G.nodes:
        seg = segment_map[sid]
        mid = _segment_midpoint(seg)
        # distance to clicked path
        d = _point_to_polyline_dist(mid, clicks_samples)
        # angle alignment
        theta_seg = _segment_heading(seg)
        theta_axis = math.atan2(axis[1], axis[0])
        ad = _angle_diff(theta_seg, theta_axis)
        # width proxy: prefer thicker (lower cost)
        w = float(seg.get("meta", {}).get("width_right", 0)) - float(
            seg.get("meta", {}).get("width_left", 0)
        )
        if not np.isfinite(w) or w <= 0:
            # conservative fallback if width not present
            w = max(1e-3, segment_length(seg["points"]) * 0.0 + 1.0)
        node_attr[sid] = {
            "mid": mid,
            "dist": d,
            "angle": ad,
            "width": w,
            "proj": _project_scalar(mid, origin, axis),
        }

    # Edge weights: symmetric from node attributes; add penalty for bridges
    for u, v, edata in G.edges(data=True):
        a, b = node_attr[u], node_attr[v]
        # combine local costs (mean of endpoints)
        dist_c = (a["dist"] + b["dist"]) * 0.5
        angle_c = (a["angle"] + b["angle"]) * 0.5
        width_c = 2.0 / (a["width"] + b["width"])  # smaller if thicker
        w = dist_weight * dist_c + angle_weight * angle_c + width_weight * width_c
        if edata.get("bridge", False):
            w += bridge_penalty
        edata["weight"] = float(w)

    # 5) Choose endpoints by projection (min/max along axis)
    nodes_sorted = sorted(G.nodes, key=lambda s: node_attr[s]["proj"]) 
    start, end = nodes_sorted[0], nodes_sorted[-1]

    # 6) Find a low-cost simple path from start to end
    try:
        path = nx.shortest_path(G, source=start, target=end, weight="weight")
    except nx.NetworkXNoPath:
        # fallback: pick the longest connected component projected extent
        ...
        if not comps:
            ...
            best_comp = max(
                comps,
                key=lambda C: (
                    max(node_attr[s]["proj"] for s in C)
                    - min(node_attr[s]["proj"] for s in C)
                ),
            )
            # take monotone chain inside the component by sorting along proj
            path = sorted(best_comp, key=lambda s: node_attr[s]["proj"])
    ...
    if not path:
        return {
            ...
            "pct_fabricated": 0.0,
            "depth_along_axis_px": _polyline_length(clicks_polyline),
            "score_parts": {"note": "no coherent path found"},
        }

    # 7) Build merged polyline and stats
    ordered_polys: List[List[List[float]]] = []
    observed_len = 0.0
    fabricated_len = 0.0
    ...
    for sid in path:
        ...
        if not P:
            continue
        ordered_polys.append(P)
        observed_len += segment_length(seg["points"])

    # Edge-level fabrication: any bridge edges between successive sids
    for u, v in zip(path[:-1], path[1:]):
        edata = G.get_edge_data(u, v)
        if edata and edata.get("bridge", False):
            # estimate fabricated length between segments as midpoint distance
            a = node_attr[u]["mid"]
            b = node_attr[v]["mid"]
            fabricated_len += float(np.linalg.norm(a - b))

    merged_poly = _concat_polylines(ordered_polys)

    depth_axis = (
        max(node_attr[s]["proj"] for s in path) - min(node_attr[s]["proj"] for s in path)
    )

    total_len = observed_len + fabricated_len
    pct_fab = 0.0 if total_len == 0 else fabricated_len / total_len

    # Score parts (lower is better); report means along nodes used
    path_dists = [node_attr[s]["dist"] for s in path]
    path_angles = [node_attr[s]["angle"] for s in path]
    path_widths = [node_attr[s]["width"] for s in path]

    score_parts = {
        "mean_dist": float(np.mean(path_dists)) if path_dists else float("inf"),
        "mean_angle": float(np.mean(path_angles)) if path_angles else float("inf"),
        "inv_mean_width": float(1.0 / np.mean(path_widths)) if path_widths else float("inf"),
        "fabricated_len": fabricated_len,
    }

    return {
        "polyline": merged_poly,
        "mask_seg_ids": path,
        "observed_length_px": observed_len,
        "fabricated_length_px": fabricated_len,
        "pct_fabricated": pct_fab,
        "depth_along_axis_px": float(depth_axis),
        "score_parts": score_parts,
    }

#--------------------------------------------------------------#

# ---- Version 2 of Annotator ----------------------------------
def annotate_taproot_polyline_tk_viz_v2(
    json_file: str | os.PathLike,
    ...
    *,
    ...
    """
    Tk-only annotator (v2): click a polyline (AL) with visible width = buffer_px pixels.
    If a list of sample_ids is provided, this function iterates through each and
    merges results into a single keyed dictionary.
    """
    ...
    if isinstance(sample_id, (list, tuple)):
        ...
        for sid in sample_id:
            ...
            single_res = annotate_taproot_polyline_tk_viz_v2(
                json_file=json_file,
                sample_id=sid,         # single sample passed
                save_to=save_to,
                invert_y=invert_y,
                buffer_px=buffer_px,
            ...
            if single_res is not None:
                results.update(single_res)   # merge keyed dicts
        return results  # <— EARLY RETURN stops execution here for multi-sample input

    # --- Single-sample logic (runs only once per sample) ---------------------
    import json, os
    from pathlib import Path
    ...
    try:
        ...
        # ---- figure setup ----
        fig = plt.figure(num=f"taproot-annotator-v2-{sample_id}", figsize=(fig_w_in, fig_h_in), dpi=dpi)
        fig.clf()
        ax = fig.add_subplot(111)
        # lc = LineCollection(seg_arrays, colors=[_VIZ.base_gray] * len(seg_arrays),
        #                     linewidths=widths, alpha=_VIZ.base_alpha, zorder=1)
        ...
        # ---- calibration overlay (shows true data-space width) ----
        ...
        # ---- interactive overlay ----
        # Compute line width in points corresponding to buffer_px data units
        ...
        # ---- densify AL for accurate geometry ----
        AL_dense = _sample_polyline(AL.tolist(), step_px=2.0)

        # ---- helpers for coverage ----
        halfw = float(buffer_px) / 2.0

        # --- Precompute AL midpoints once outside the segment loop ---
        AL_dense = np.asarray(AL_dense, float)
        AL_midpoints = 0.5 * (AL_dense[:-1] + AL_dense[1:])
        tree = cKDTree(AL_midpoints)
        
        def _angle_deg(u, v):
            """Return folded angle between vectors u, v (0–90°, direction-agnostic)."""
            nu, nv = np.linalg.norm(u), np.linalg.norm(v)
            if nu == 0 or nv == 0:
                return None
            c = np.clip(np.dot(u, v) / (nu * nv), -1, 1)
            ang = math.degrees(math.acos(c))
            return min(ang, 180.0 - ang)   # treat reversed directions as equivalent

        
        def _dist_points_polyline(pts: np.ndarray, poly: np.ndarray) -> np.ndarray:
            """Vectorized distances from many points to a polyline."""
            diffs = np.diff(poly, axis=0)
            v2 = (diffs ** 2).sum(axis=1)
            v2[v2 == 0] = 1
            d = pts[:, None, :] - poly[None, :-1, :]
        ...
        def _coverage_frac(segment_points: list[list[float]], step_px: float = 3.0) -> float:
            """Approx length coverage: fraction of sampled points within corridor."""
        ...
        def _any_inside(segment_points: list[list[float]]) -> bool:
            """Quick coarse check before detailed coverage test."""
            S = np.asarray(segment_points, float)
            ...
            L = float(cum[-1])
            if L <= 0: return False
            n = max(2, int(round(L / step_coarse)) + 1)
            t = np.linspace(0.0, L, n)
            idx = np.searchsorted(cum, t, side="right") - 1
            idx = np.clip(idx, 0, len(d) - 1)
            alpha = (t - cum[idx]) / np.maximum(d[idx], 1e-6)
        ...
        for sid, seg in segmap.items():
            ...
            dist = _dist_points_polyline(np.asarray(seg["points"], float), AL_dense)
            min_dist = dist.min()
            ...
            if cov >= overlap_threshold or min_dist <= (halfw + 0.5 + 0.5 * seg_width):
                ...
                # --- compute local tangent angle relative to nearby part of annotated line ---
                centroid = P.mean(axis=0)
                _, j = tree.query(centroid)          # nearest AL midpoint index
                v_al = AL_dense[j+1] - AL_dense[j]   # local tangent of annotation
                v_seg = P[-1] - P[0]                 # end-to-end vector of segment
            
                ang = _angle_deg(v_seg, v_al)
                if (ang is None) or (ang <= angle_max):   # same threshold as before
                    kept_ids.append(sid)



        # ---- save sidecar JSON (JSON when saved, in a keyed dictionary when used in memory) ----
        sidecar_path = Path(save_to)
        ...
        # Remove any existing entry for this sample_id
        ...
        # Save back to disk as list-based JSON
        sidecar_path.write_text(json.dumps({"entries": entries}, indent=2), encoding="utf-8")
        
        # Return keyed dictionary form in memory
        return {
            sample_id: new_entry
        }
        ...
        if save_png_dir:
            ...
            _auto_limits(ax2, seg_arrays)
            ax2.set_aspect("equal"); ax2.axis("off")
            lw_pts2 = (float(buffer_px) * 0.5) * 72.0 / float(dpi)
            ...
            out_png = Path(save_png_dir) / f"{sample_id}__taproot_gt_v2.png"
            fig2.savefig(out_png, dpi=dpi, bbox_inches="tight")
            plt.close(fig2)
        ...
        return {
            sample_id: {
                ...
                "polyline": AL.tolist(),
                "seg_ids": kept_ids,
                "meta": {"buffer_px": buffer_px, "overlap_threshold": overlap_threshold},
            }
        }
